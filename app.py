...
# Disable SSL warnings for the PLA site if necessary
...
def get_cached(key, fetch_fn, ttl_seconds):
    """Handles logic for returning fresh data or cached data if API fails."""
    ...
    try:
        ...
        # Fallback to expired cache if available (Last Known Good)
        if key in _cache:
            return _cache[key]['data'], _cache[key]['fetched_at']
        return None, ''
...
def get_pla_flag():
    ...
    now = datetime.now(LONDON_TZ)
    # Trigger actual fetch only at specific times or if cache is empty
    should_refresh = (now.hour in [6, 19] and now.minute in [1, 5])
    
    if should_refresh or 'pla_flag' not in _cache:
        data, t = get_cached('pla_flag', fetch, ttl_seconds=0) # 0 force-refreshes get_cached logic
        return data, t
...
def build_dashboard_data():
    ...
    # Offset for local time presentation
    ...
    if tides:
        ...
        # Check for boat line warning (60 mins after Low Water)
        try:
