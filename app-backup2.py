...
try:
    ...
    _WEBSOCKET_OK = False
    print("WARNING: websocket-client not installed — Blitzortung lightning disabled. pip install websocket-client")
...
# ---------------------------------------------------------------------------
# Lightning — Blitzortung real-time strike counter
# ---------------------------------------------------------------------------
# Connects to the Blitzortung WebSocket, filters strikes within
# LIGHTNING_STRIKE_RADIUS_KM of the club, and keeps a 30-min rolling count.
# Non-commercial / personal use only per Blitzortung terms.
# ---------------------------------------------------------------------------

LIGHTNING_STRIKE_RADIUS_KM = 25
LIGHTNING_STRIKE_WINDOW_SECS = 1800  # 30 minutes
...
def _decode_blitzortung(b):
    """LZW decode of obfuscated Blitzortung WebSocket messages."""
    if isinstance(b, str):
        b = b.encode()
    e = {}
    d = list(b.decode(errors='replace'))
    if not d:
        return b''
...
def _blitzortung_thread():
    ...
    while True:
        ...
        print(f"Blitzortung: connecting to {url}")
        try:
            ws = _websocket_lib.WebSocketApp(
                url,
                on_open=_blitz_on_open,
            ...
            print(f"Blitzortung connection error: {e}")
        idx += 1
        time_mod.sleep(5)


def get_strike_count_30min():
    """Return count of nearby strikes in the last 30 minutes, pruning stale entries."""
    now = time_mod.time()
    cutoff = now - LIGHTNING_STRIKE_WINDOW_SECS
    with _strike_lock:
        while _strike_deque and _strike_deque[0] < cutoff:
            _strike_deque.popleft()
        return len(_strike_deque)


# Start background listener (daemon — exits with main process)
_blitz_thread = threading.Thread(target=_blitzortung_thread, daemon=True)
_blitz_thread.start()
...
@app.route("/api/lightning-strikes")
def lightning_strikes_endpoint():
    """Real-time Blitzortung strike count within 25 km of Hammersmith (last 30 min)."""
    count = get_strike_count_30min()
    return jsonify({
        "count_30min":  count,
        "radius_km":    LIGHTNING_STRIKE_RADIUS_KM,
        "window_mins":  LIGHTNING_STRIKE_WINDOW_SECS // 60,
        "source":       "Blitzortung",
